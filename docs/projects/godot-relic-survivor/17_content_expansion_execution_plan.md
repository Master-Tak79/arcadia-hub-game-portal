...
### 범위
- 캐릭터 전용 스킬/무기 트리 노드 구조 설계
- 트리 포인트/해금 규칙/저장 모델 설계
- QA 토큰/시나리오(`tree_*_loop`) 설계

### 완료 기준
- 설계 문서 확정 + 구현 순서 고정
- 정책 결정 반영 완료(재화/단가/적용시점)

### 리스크
- 메타/캐릭터/무기/액티브 보정 중첩으로 튜닝 복잡도 상승
- 트리 UI 정보 과밀 가능성
...
### 범위
- CC0 에셋(플레이어/적/투사체/배경) 1차 도입
- 스프라이트 렌더링 + 기존 폴백 경로 유지
- 자산 출처/라이선스 등록(`09_asset_register.md`)

### 완료 기준
- 핵심 오브젝트 스프라이트 렌더링 정상
- headless/누수/체크포인트 검증 통과

### 리스크
- 아트 스타일 혼합으로 일부 UI/효과와 톤 미스매치 가능
- 추후 애니메이션/VFX 단계에서 재튜닝 필요
