...
## 2026-03-03 (Step 14 Fast Follow)

### 미션 리워드 피드백 튜닝
- 미션 연속 성공 시 스트릭 보너스 EXP 추가
  - 보너스: `(streak-1) * 2`, 최대 +8
- 미션 실패 시 스트릭 리셋
...
## 2026-03-03 (Step 18 VFX/Animation Polish)

### 체감 개선 목표
- 피격/처치 순간 피드백을 더 명확하게 전달
- 상단 이벤트 배너와 레벨업 패널 등장 시 시선 유도 강화

### 변경 메모
- 밸런스 수치 변경 없음(표현/연출 레이어만 보강)
- 자동검증(게이트/누수/프리즈) 회귀 없음 확인

## 2026-03-03 (Step 19 UI/Graphics Overhaul)

### 변경 요약
- 밸런스 수치 변경 없음
- UI/연출 레이어 상향에 집중(HUD 정보계층, 배경 시인성)
...
## 2026-03-05 (Step 22-A Content/Fun Expansion)

### 변경 요약
- 패턴 변주: 보스 CROSS 소환 추가
- 이벤트 안정화: 동일 이벤트 반복 완화 + 저체력/고압박 시 shock 비중 완화
- 시너지 강화: 유물 세트 보너스 발동(역할별)
