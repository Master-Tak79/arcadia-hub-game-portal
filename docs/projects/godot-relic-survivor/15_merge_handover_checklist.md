...
- [ ] menu idle probe 3회 완료(평균 6.2/6.3/6.2), 플레이 구간 재실측 필요
...
2. merge 전 최신 자동검증 실행:
   ...
   - (포함 시나리오: `boss_pattern`, `boss_phase2`, `elite_loop`, `relic_loop`, `event_loop`, `feel_loop`, `mission_loop`, `elite_variant_loop`, `character_ranger`, `character_warden`, `active_ranger`, `active_warden`, `tree_ranger`, `tree_warden`, `tree_ui`, `weapon_pierce`, `weapon_dot`, `weapon_aoe`, `meta_loop`)
3. 필요 시 leak 추적:
...
- [ ] 다음 마일스톤(캐릭터/무기 트리 고도화) 작업 브랜치 분기
- [ ] 잔여 리스크(연출 폴리싱/체감 난이도 미세조정) 이관
- [ ] 수동 QA 결과 문서 반영 (`04`, `05`, `06`, `13`)

## 메뉴/옵션 수동 확인
- [ ] 타이틀 메뉴 OPTIONS 진입/복귀 확인
- [ ] 설정 변경 즉시 반영(SFX/카메라/창모드) 확인
- [ ] 재실행 후 설정 복원 확인
