...
- [ ] 재실행 시 설정 저장/복원 동작 확인 (수동 QA 시 확인)
- [ ] 유물 세트 보너스 발동 체감(`RELIC_SET_BONUS:*`) 확인 (수동 QA 시 확인)
- [ ] 보스 소환 패턴 연속 반복 빈도 완화 체감 확인 (수동 QA 시 확인)
- [ ] 보스 소환 직후 recovery window 체감 확인 (수동 QA 시 확인)
- [ ] 이벤트 반복 완화 체감(동일 이벤트 연속 빈도) 확인 (수동 QA 시 확인)
- [ ] 보스 CROSS 소환 패턴 가독성/회피 가능성 확인 (수동 QA 시 확인)
- [ ] OPTIONS 메뉴 SFX/카메라/창모드 변경 동작 확인 (수동 QA 시 확인)
