...
function endGame(reason = "shift-end") {
  ...
  showOverlayUI(
    { overlay, overlayTitle, overlayText },
    "라운드 종료",
    `${reasonText}\n점수 ${Math.floor(state.score)} · 배차 ${state.dispatches} · 최고 ${state.best}`
  );
...
function doAction(type) {
  ...
  if (type === "north" || type === "central" || type === "south") {
    ...
    if (result.ok) {
      const labels = { north: "북부선", central: "중앙선", south: "남부선" };
  ...
  if (type === "dispatch") {
    ...
    if (result.ok) {
      showNotice(`🚆 배차 완료 +${result.creditGain}₡`, 760);
      sfx.play("tick");
      vibrate(6);
    } else {
      showNotice("배차 자원이 부족합니다", 620);
      sfx.play("hit");
    }
    syncHud();
    return;
  ...
  if (type === "overdrive") {
    ...
    if (result.ok) {
      showNotice("🚄 OVERDRIVE", 960);
      sfx.play("start");
      vibrate([8, 12, 8]);
    } else if (result.reason === "cooldown") {
      showNotice("오버드라이브 쿨다운 중", 620);
      sfx.play("hit");
    } else if (result.reason === "insufficient-credit") {
      showNotice(`크레딧 부족 (${result.cost})`, 620);
      sfx.play("hit");
