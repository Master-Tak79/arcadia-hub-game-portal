...
export function createRenderer({ canvas, ctx }) {
  ...
  function render(state) {
    ...
    drawRailLane(ctx, x, 112, panelW, "North Line", state.northLv, northProgress, "#8be3ff");
    drawRailLane(ctx, x, 226, panelW, "Central Line", state.centralLv, centralProgress, "#ffd88e");
    drawRailLane(ctx, x, 340, panelW, "South Line", state.southLv, southProgress, "#9dffbb");
    ...
    if (state.overdriveMs > 0) {
      ...
      ctx.fillText(`🚄 OVERDRIVE ${remain}s`, canvas.width * 0.5, 708);
      ctx.textAlign = "start";
    }
