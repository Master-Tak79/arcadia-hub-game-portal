...
function endGame() {
  ...
  showOverlayUI(
    { overlay, overlayTitle, overlayText },
    "게임 오버",
    `최종 점수 ${Math.floor(state.score)}점 · 격추 ${state.kills} · 최고 ${state.best}`
  );
  pauseBtn.textContent = "일시정지";
}
...
function doNova() {
  ...
  if (result.cleared > 0) {
    showNotice(`💫 NOVA ${result.cleared}기 정리`, 760);
    sfx.play("start");
    vibrate([8, 12, 8]);
  } else {
    showNotice("💫 NOVA 충전 샷", 620);
    sfx.play("tick");
  }
...
showOverlayUI(
  { overlay, overlayTitle, overlayText },
  "Void Raiders",
  "자동 사격으로 적을 격추하며 생존하세요.\nNOVA 타이밍이 대량 정리의 핵심입니다."
);
