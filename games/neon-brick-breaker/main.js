...
import { launchBall as launchBallSystem, resetRound, stepGame } from "./systems.js";
import {
  createBall,
  createPaddle,
  createState,
...
const bricks = resetRound(state, paddle, ball);
...
function startGame() {
  ...
  const fresh = resetRound(state, paddle, ball);
  bricks.length = 0;
  bricks.push(...fresh);
...
function launchBall() {
  if (!state.running || state.gameOver) return;
  const launched = launchBallSystem(state, ball, state.level);
  if (!launched) return;
  showNotice("🚀 발사", 600);
  sfx.play("start");
  vibrate(8);
...
function endGame() {
  ...
  showOverlayUI(
    { overlay, overlayTitle, overlayText },
    "게임 오버",
    `최종 점수 ${state.score}점 · 최고 점수 ${state.best}점\n시작 버튼으로 새 라운드를 시작하세요.`
  );
  pauseBtn.textContent = "일시정지";
}
...
function frame(now) {
  ...
  stepGame({
    ...
    callbacks: {
      ...
      onLifeLost: (left) => {
        showNotice(`💔 남은 체력 ${left}`, 980);
        sfx.play("hit");
        vibrate([8, 20, 8]);
      },
      onLevelUp: (level) => {
        showNotice(`⬆️ LEVEL ${level}`, 1100);
        sfx.play("start");
        vibrate([8, 12, 8]);
      },
      onMissionComplete: () => {
  ...
  renderer.render(state, paddle, ball, bricks);
  syncHud();
...
showOverlayUI(
  { overlay, overlayTitle, overlayText },
  "Neon Brick Breaker",
  "패들을 움직여 공을 튕기고 모든 브릭을 파괴하세요.\n레벨 3 도달 시 미션 보너스를 획득합니다."
);
